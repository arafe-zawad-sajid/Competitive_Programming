...
public class Main {
    ...
    // bitwise sieve
    public static int bitsieve(int max) {
        ...
        int count = 1;
        for(int i=3; i<=max; i+=2) {  // starting from 3, skipping all even numbers
            if(check(status[i>>5], i&31)==0) {  //if prime
                count++;
            }
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        int count = bitsieve(n);
        bw.write(count+"\n");

        while(q-- > 0) {
            int x = rd.readInt();
            if(x==2 || (x>2 && x%2!=0 && (check(status[x>>5], x&31))==0)) {  //if prime
                bw.write("1\n");
            }
            else {
                bw.write("0\n");
